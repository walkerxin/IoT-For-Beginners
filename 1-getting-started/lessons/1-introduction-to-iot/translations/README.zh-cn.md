...
为了形容运用传感器（sensors，又译作感应器）来连接网络与物理世界，1999 年 [凯文·阿什顿（Kevin Ashton）](https://wikipedia.org/wiki/Kevin_Ashton) 创造了“物联网（IoT）”一词。自从那时起，这个术语常常被用来形容任何能够与周围物理世界交互的设备。这些设备可以使用传感器（sensors）收集数据，或者使用执行器（actuators，指的是执行诸如打开开关，点亮发光二极管等操作的设备）与物理世界进行交互。通常执行器会连接到其它设备或者互联网。

> **传感器** 从外界中收集数据，例如：测量速度、温度或地点。
>
> **执行器** 将电信号转换成现实世界的交互，例如：触发开关，打开灯，发出声音或者向其它硬件发送控制信号，例如，打开电源插座。

物联网作为一个技术领域，不仅是设备，它也包含云服务；这些服务能处理传感器数据，或者将请求发送给跟物联网设备有连接的执行器。它也包含那些没有或者不需要互联网连接的设备，通常被称为“边缘设备（edge devices）”。这些设备有能力用基于云的 AI 模型自己处理与回应传感器的数据。

物联网是一个快速发展的技术领域。专家预计到 2020 底，世界上有`三百亿物联网设备`部署并连接到互联网。专家也预计到 2025 年，物联网设备将收集大约 80 ZB（80 万亿 GB）的数据。这是巨量的数据！

![这个图表展示随着时间的推移的有源 IoT 设备；它展示出一个上升趋势，从 2015 年不超过 50 亿到 2025 年超过 300 亿](../../../../images/connected-iot-devices.svg)

✅ 做个小调查： 物联网设备收集的数据中，有多少是被实际使用了？有多少是被浪费了？为什么那么多数据被忽略了呢？

数据是物联网成功的关键。要想成为一名成功的物联网开发者，你有必要了解你需要收集的数据、如何收集它，如何利用它来作出决策，以及必要时，如何利用这些决策与物理世界进行交互。

## 物联网设备

IoT 的 **T** 代表 **Things**（物）—— 可以和周边物理世界交互的设备；它们使用传感器收集数据或者使用执行器与真实世界进行交互，完成任务。

用于生产或商业用途的设备（例：健身追踪器、工业机器控制器等）通常是定制的。它们使用定制的电路板——甚至有可能是定制的处理器——旨在满足特定任务的需求。例：要戴在手上的需要够小，或者要承受高温度、高压力、高振动的工厂环境的需要够耐用。

无论你正在学物联网或是在创建原型设备，作为一名 物联网开发者，`你需要由一个开发者套件（developer kits）开始`。这些是为物联网开发者设计的通用设备，通常具有生产设备上没有的功能，例如一组外部引脚，用来连接传感器或执行器，支持硬件调试，或者附加资源（在进行大规模生产运作时会增加不必要的开销）。

这些开发者套件通常有两种：`微控制器（microcontrollers）和单板机（single-board computers）`。我们会在这儿介绍它们，而将在下一课更详细地解释它们。

> 💁 你的手机也算是一个通用物联网设备；它拥有内置的传感器和执行器，（其中）不同的应用程序会以不同的方式和不同的云服务来使用这些传感器和执行器。你甚至可以找到一些是使用手机应用作为物联网设备的物联网教程。
...
💾 内存（随机存取存储器（RAM）和程序存储器 —— 储存你的程序、数据和变量的地方

🔌 可编程输入/输出（I/O）连接 —— 为了与连接的外部设备（如传感器或执行器）进行通信

微控制器通常是较便宜的计算设备；定制硬件的平均成本下降到 0.50 美元，而有些设备低至 0.03 美元。开发者套件的起价低至 4 美元，但加上越多功能，价钱就越高。[Wio Terminal](https://www.seeedstudio.com/Wio-Terminal-p-4509.html) 是个来自 [Seeed studios](https://www.seeedstudio.com) 的微控制器开发套件；它包含传感器、执行器、Wi-Fi 和一个屏幕，总共算起来大约 30 美元。
...
微控制器被设计成（为其编写程序来）`完成有限数量且非常特定的任务`，不像 PC 或 Mac 那样的通用计算机。除了一些很特殊的场景，你无法连接显示器、键盘和鼠标并利用它完成通用任务。

微控制器开发者套件通常配有额外的传感器和执行器。大多数电路板会有至少一个可编程的发光二极管（LEDs），还有其它设备，例如使用不同制造商的生态系统或内置传感器（常见的如 温度传感器）来添加更多传感器或执行器的标准插头。有些微控制器有内置的无线连接，如 蓝牙 或 Wi-Fi，或是在电路板上有额外的微控制器来添加这种连接。

> 💁 我们通常用 C 或 C++ 来为微控制器写程序。

### 单板机

单板机是一种小型计算设备；`它把一个完整计算机的所有要素都装在了一个小板上`。这些设备的规格与台式电脑或笔记本电脑比较相近，运行着完整的操作系统，但体积小、耗电少，而且便宜得多。

![一个 Raspberry Pi 4](../../../../images/raspberry-pi-4.jpg)

树莓派（Raspberry Pi）是最受欢迎的单板机之一。

与微控制器一样，单板机具有 CPU、内存和输入/输出引脚，`但它们有额外的功能`，如一个图形芯片（让你能连接显示器、音频输出），和一些 USB 端口（让你能连接键盘、鼠标和其它标准的 USB 设备，如网络摄像头和外置储存）。程序与操作系统一起存储在 SD 卡或硬盘驱动器上，而不是内置于板中的内存芯片。

> 🎓 你可以把单板机当成一个更小、更便宜的个人电脑，就像你现在正在用来读这篇文章的 `PC 或 Mac。可是，单板机还增加了通用输入/输出引脚（GPIO，general-purpose input/output），使其能与传感器、执行器交互。`

单板机是功能齐全的计算机，所以你可以用任何编程语言来为它写程序。我们通常用 Python 为物联网设备写程序。

### 日后课程的硬件选择

后续的所有课程，包括用物联网设备与物理世界进行交互以及云通信的作业。每节课会支持3种设备选择：
1. Arduino（使用一个 Seeed Studios Wio Terminal）
2. 物理单板机 - 物理设备（树莓派四代，Raspberry Pi 4），
3. 虚拟单板机 - 在你的电脑上运行的虚拟单板机。 

你能在[硬件手册](../../../../hardware.md)查到完成作业所需的硬件。

> 💁 你不需要为了完成作业而买任何物联网硬件；虚拟单板机即可完成所有任务。

要使用哪种硬件是你的选择，取决于你家里或学校里有什么，以及你知道或想学的编程语言。两种硬件都使用同样的传感器生态系统，所以万一你想途中改变你的选择，你也不需要替换大部分的套件。用虚拟单板机相当于在 Raspberry Pi 上学习，如果你最后购买了设备和传感器，大部分代码都可以转移到这个（物理）Pi 上。

### Arduino 开发者套件

如果你对微控制器的开发感兴趣，那你可以用一个 Arduino 设备完成作业。你需要对 C 或 C++ 的编程语言有基本的理解，因为将来的课程只会教关于 Arduino 框架的程序、需要用到的感应器和执行器以及跟云交互的库。

作业将用 [Visual Studio Code](https://code.visualstudio.com/?WT.mc_id=academic-17441-jabenn)  跟 [（微控制器开发的）PlatformIO 扩展](https://platformio.org)。如果你对 Arduino IDE 熟悉的话，你也能用它，但我们不会提供指示。
...
如果你使用的是树莓派，则可以使用树莓派操作系统的完整桌面版来运行你的 PI，并直接在你的树莓派上使用 [树莓派操作系统版 VS Code](https://code.visualstudio.com/docs/setup/raspberry-pi?WT.mc_id=academic-17441-jabenn)编写程序。或者把它当成一个无头设备，从你的电脑，利用 VS Code 的 [Remote SSH 插件](https://code.visualstudio.com/docs/remote/ssh?WT.mc_id=academic-17441-jabenn) 连接到你的树莓派上，就可以去编写、调试和运行程序，就像你直接在树莓派上写程序一样。
...
> 💁 如果你还没有设备，参阅[硬件手册](../../../../translations/hardware.zh-cn.md) 帮你决定你要用的设备，以及你需要购买的额外硬件。当然硬件不是必需购买的，因为你可以用虚拟硬件运行所有的项目。
...
✅ 做个小调查：对于以下描述的每个领域，找到一个文本中没有给出的具体例子。
...
如果地球要养活不断增长的人口，数字农业就非常重要，尤其是对于依靠[自给农业](https://wikipedia.org/wiki/Subsistence_agriculture) 生存的 5 亿家庭中的 20 亿人而言。数字农业涵盖从几块钱的传感器，到大规模的商业装置。农民可以首先监测温度以及用[生长度日（GDD）](https://wikipedia.org/wiki/Growing_degree-day)，预测农作物什么时候收割。他们还可以将土壤湿度监测与自动浇水系统连接起来，为他们的植物提供刚好所需的水量，而不浪费水资源。最后，农民可以更进一步，用无人驾驶飞机、卫星数据、人工智能来监测大面积农田的作物生长、疾病和土壤质量。
